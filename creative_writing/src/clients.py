...
def _model_log_path(model: str) -> Path:
    """Return a model-specific log file path, e.g. logs/llm_calls_gemma-4-E4B-it.jsonl"""
...
class WolverineClient:
    ...
    def _append_jsonl_log(self, entry: dict) -> None:
        """Append one JSON record to the per-call log file."""
        try:
    ...
    def chat(self, *, system_prompt: str, user_prompt: str, request_tag: str | None = None) -> str:
        ...
        print(
            ...
            f"(tag={request_tag}, latency_ms={latency_ms}, prompt_tokens={prompt_tokens}, "
