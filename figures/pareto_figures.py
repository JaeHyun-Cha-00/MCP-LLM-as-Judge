...
def _sw_pretty(stem: str) -> str:
    ...
    s = s.replace("-BF16", "").replace("-bf16", "")
    # Explicit name normalisation for known models
    s = s.replace("nvidia-nemotron3-nano-4b", "Nemotron-3-Nano-4B")
    s = s.replace("llama-3.2-3B", "Llama-3.2-3B")
    s = s.replace("qwen3.5-4B", "Qwen3.5-4B")
    s = s.replace("qwen3-4B", "Qwen3-4B")
    return s


def _cw_pretty(stem: str) -> str:
    s = stem.replace("_result", "")
    s = s.replace("-InstructMode-General", "")  # must come before generic -Instruct strip
...
DATASETS = [
    DatasetConfig(
        ...
        baseline_path=PROJECT_ROOT / "story_writing_benchmark" / "dataset" / "results"
                      / "claude_sonnet_4.6.csv",
...
def main():
    ...
    p.add_argument("--output-dir", type=Path,
                   default=PROJECT_ROOT / "neurips_paper2" / "figures",
                   help="Where to save PDF/PNG files (default: neurips_paper2/figures)")
